...
public partial class frmSelfAssessmentRating : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            hdnNodeID.Value = Session["EmpNodeID"].ToString();
            hdnbandId.Value = Session["BandId"].ToString();
            frmGetUserDetail(hdnNodeID.Value, "0");
        }
    }
    ...
    private void frmGetUserDetail(string EmpNodeID,string CycleId)
    {
        ...
        {
            using (SqlCommand command = new SqlCommand("spRspGetSelfFeedbackResponses", Scon))
            {
    ...
    public static string fnSave(object udt_DataSaving, string LoginId, string RspID, string Status)
    {
        try
        {
            ...
            SqlCommand Scmd = new SqlCommand();
            Scmd.Connection = Scon;
            Scmd.CommandText = "[spUpdateSelfFeedbackResponses]";
            Scmd.CommandType = CommandType.StoredProcedure;
